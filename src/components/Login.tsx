    ...
    try {
      ...
      if (error) {
        if (error.message.includes('Invalid login credentials')) {
          setError('Magaca isticmaalaha ama furaha sirta ah ayaa qaldan - Invalid email or password');
        } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-100 flex items-center justify-center p-4">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute top-3/4 right-1/4 w-72 h-72 bg-purple-200 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-float" style={{animationDelay: '2s'}}></div>
        <div className="absolute bottom-1/4 left-1/3 w-80 h-80 bg-indigo-200 rounded-full mix-blend-multiply filter blur-xl opacity-70 animate-float" style={{animationDelay: '4s'}}></div>
      </div>
      ...
      <div className="relative max-w-md w-full">
        ...
        <div className="mt-6 bg-white/60 backdrop-blur-lg rounded-xl border border-white/20 p-6 animate-fadeIn">
            ...
            {staffAccounts.map((account, index) => (
              <div key={index} className="flex items-center justify-between p-3 bg-white/40 rounded-lg border border-white/30">
                ...
                <div className="text-right">
                  <p className="text-sm font-mono text-slate-700">{account.email}</p>
                </div>
              </div>
            ))}
          </div>
          <div className="mt-4 text-center text-sm text-slate-600">
            ...
            <p className="text-xs mt-1">La xiriir Hassan si aad u hesho furaha sirta ah</p>
          </div>
        </div>
      </div>
    </div>
