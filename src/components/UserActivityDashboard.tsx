...
import { Users, Clock, Calendar, Activity, UserCheck, UserX } from 'lucide-react';
...
export default function UserActivityDashboard() {
    ...
    try {
      ...
      const activities: UserActivity[] = profiles?.map(profile => ({
        ...
        is_online: profile.last_login ? 
          (new Date().getTime() - new Date(profile.last_login).getTime()) < 30 * 60 * 1000 : false // Online if logged in within 30 minutes
      })) || [];
  ...
  function formatLastSeen(lastSignIn: string | null) {
    ...
    const diffInMinutes = Math.floor((now.getTime() - signInDate.getTime()) / (1000 * 60));
    
    if (diffInMinutes < 1) return 'Hadda - Just now';
    if (diffInMinutes < 60) return `${diffInMinutes} daqiiqo ka hor - ${diffInMinutes} minutes ago`;
    
    const diffInHours = Math.floor(diffInMinutes / 60);
    if (diffInHours < 24) return `${diffInHours} saac ka hor - ${diffInHours} hours ago`;
    
    const diffInDays = Math.floor(diffInHours / 24);
    if (diffInDays < 7) return `${diffInDays} maalmood ka hor - ${diffInDays} days ago`;
  ...
  function getRoleColor(role: string) {
    switch (role) {
      case 'admin': return 'bg-gradient-to-r from-purple-500 to-blue-600';
      case 'owner': return 'bg-gradient-to-r from-purple-500 to-blue-600';
      case 'staff': return 'bg-gradient-to-r from-green-500 to-teal-600';
      ...
      <div className="bg-white/80 backdrop-blur-lg rounded-2xl shadow-xl border border-white/20 p-6">
        <div className="text-center">
          ...
          <p className="text-slate-600">
            Kaliya admin ayaa arki kara faallooyinka isticmaalayaasha - Only admin can view user activities
          </p>
        </div>
      </div>
    );
            ...
            {userActivities.map((activity) => (
              ...
              >
                <div className="flex items-center space-x-4">
                  {/* Avatar */}
                  <div className="relative">
                    <div className={`absolute inset-0 ${getRoleColor(activity.role)} rounded-full blur opacity-50`}></div>
                    <div className={`relative ${getRoleColor(activity.role)} p-3 rounded-full`}>
                  ...
                  <div>
                    <div className="flex items-center space-x-2">
                      ...
                      <span className={`px-2 py-1 text-xs font-semibold rounded-full ${
                        activity.role === 'admin' ? 'bg-purple-100 text-purple-800' : 'bg-green-100 text-green-800'
                      }`}>
                    ...
                    <p className="text-sm text-slate-600 font-mono">{activity.email}</p>
                  </div>
                </div>
        ...
        <div className="bg-white/80 backdrop-blur-lg rounded-xl shadow-lg border border-white/20 p-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-slate-800">
                {userActivities.filter(u => u.last_sign_in && 
                  (new Date().getTime() - new Date(u.last_sign_in).getTime()) < 24 * 60 * 60 * 1000
                ).length}
              </p>
            </div>
          </div>
