...
export default function Dashboard() {
  ...
  const [topProducts, setTopProducts] = useState<Array<{ product: Product; total: number }>>([]);
          ...
          <div className="space-y-3">
            {topProducts.length === 0 ? (
              ...
              topProducts.map((item, index) => (
                <div key={item.product.id} className="flex items-center space-x-3 p-3 bg-slate-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-slate-800">KES {item.total.toLocaleString()}</p>
                  </div>
                </div>
              ))
            )}
            ...
            {recentSales.length === 0 ? (
              ...
              recentSales.map((sale) => (
                <div key={sale.id} className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-slate-800">KES {sale.total_sale.toLocaleString()}</p>
                    <p className="text-sm text-green-600">+KES {sale.profit.toLocaleString()}</p>
                  </div>
                </div>
              ))
            )}
