# 🚀 How to Get Hassan Bookshop on Google Search

Your website is currently **NOT indexed by Google yet**. Here's exactly what you need to do:

---

## ⚡ IMMEDIATE ACTIONS (Do These NOW!)

### 1. Submit to Google Search Console ⭐ **MOST IMPORTANT**

**Steps:**

1. Go to: https://search.google.com/search-console
2. Click "Start Now" and sign in with a Google account
3. Click "Add Property"
4. Enter: `https://hassan.lenzro.com`
5. Choose verification method:

   - **HTML Tag Method** (Easiest):
     - Copy the meta tag Google gives you
     - I'll add it to your index.html file
   - **OR DNS Method**:
     - Add TXT record to your domain DNS settings

6. After verification, submit your sitemap:

   - In Search Console, go to "Sitemaps"
   - Enter: `https://hassan.lenzro.com/sitemap.xml`
   - Click "Submit"

7. Request Indexing:
   - Go to "URL Inspection" in Search Console
   - Enter: `https://hassan.lenzro.com`
   - Click "Request Indexing"

**Timeline:** Google will start crawling within 24-48 hours

---

### 2. Create Google Business Profile ⭐ **CRITICAL FOR LOCAL SEARCH**

This is **ESSENTIAL** for appearing in "bookshop eastleigh" searches!

**Steps:**

1. Go to: https://business.google.com
2. Click "Manage Now" and sign in
3. Enter your business name: **Hassan Bookshop**
4. Choose category: **Book Store**
5. Add location: **Global Apartments, Section One, Eastleigh, Nairobi**
6. Mark on map exactly where you are
7. Add contact: **+254 722 979 547**
8. Add website: **https://hassan.lenzro.com**
9. Add hours: **Monday-Sunday, 8:00 AM - 8:00 PM**
10. Upload photos:
    - Storefront (exterior)
    - Interior shots
    - Product photos
    - Logo
11. Complete your profile 100%

**Timeline:** Appears on Google Maps within 1-3 days after verification

---

### 3. Submit to Bing (Microsoft) Search

**Steps:**

1. Go to: https://www.bing.com/webmasters
2. Sign in with Microsoft account
3. Add your site: `https://hassan.lenzro.com`
4. Verify ownership
5. Submit sitemap: `https://hassan.lenzro.com/sitemap.xml`

---

## 📱 SOCIAL MEDIA SETUP (Helps with SEO)

Create business profiles on:

### Facebook Business Page

- Name: Hassan Bookshop Eastleigh
- Category: Bookstore
- Location: Eastleigh, Nairobi
- Phone: +254 722 979 547
- Website: https://hassan.lenzro.com
- Post daily updates

### Instagram Business

- @hassanbookshop
- Link website in bio
- Post product photos daily
- Use hashtags: #NairobiBooks #EastleighBookshop #KenyaBooks

### WhatsApp Business

- Number: +254 722 979 547
- Create catalog of products
- Add business hours
- Link to website

---

## 🎯 LOCAL SEO ACTIONS

### Get Listed on Kenyan Directories

**Submit your business to:**

1. **Google Maps** (via Google Business Profile above)
2. **PigiaMe Kenya**: https://www.pigiame.co.ke
3. **Jiji Kenya**: https://jiji.co.ke
4. **BrighterMonday**: https://www.brightermonday.co.ke
5. **Nairobi Business Directory**
6. **Kenya Yellow Pages**
7. **Yelp Kenya**

---

## 📊 Verify Your Website is Live

**Test these URLs right now:**

✅ Homepage: https://hassan.lenzro.com
✅ Sitemap: https://hassan.lenzro.com/sitemap.xml
✅ Robots: https://hassan.lenzro.com/robots.txt

---

## 🔥 Quick SEO Boost Tips

### Get Your First Reviews

- Ask 10 customers to review on Google Business Profile
- Reviews = Higher rankings!

### Post Content

- Create a blog section
- Write about: "Best Textbooks in Nairobi"
- Post on social media with location tags

### Local Keywords

Use these in your content:

- "bookshop eastleigh nairobi"
- "books eastleigh section one"
- "textbooks nairobi eastleigh"
- "hassan bookshop eastleigh"
- "bookstore near eastleigh mosque"

---

## ⏱️ TIMELINE EXPECTATIONS

| Action                           | When You'll See Results             |
| -------------------------------- | ----------------------------------- |
| Google Search Console submission | 24-48 hours (indexing starts)       |
| Google Business Profile          | 1-3 days (appears on Maps)          |
| Appear in search results         | 1-2 weeks                           |
| Rank in top 10                   | 1-3 months (with consistent effort) |
| Local pack (Map results)         | 2-4 weeks (with Google Business)    |

---

## 🎓 WHY YOU'RE NOT SHOWING UP YET

Your website is **brand new** and Google doesn't know it exists yet because:

1. ❌ Not submitted to Google Search Console
2. ❌ No Google Business Profile
3. ❌ No backlinks from other websites
4. ❌ Website just launched (December 2025)
5. ❌ No social media presence yet

**Once you complete the steps above, you WILL appear in search!**

---

## 📞 NEED HELP?

If you need assistance with:

- Google Search Console verification
- Google Business Profile setup
- Adding the verification meta tag

Let me know and I'll help you add it to your website!

---

## 🚀 PRIORITY ORDER (Do in this sequence)

1. **TODAY:** Submit to Google Search Console + Request Indexing
2. **TODAY:** Create Google Business Profile
3. **THIS WEEK:** Get 5 customer reviews on Google
4. **THIS WEEK:** Create Facebook & Instagram pages
5. **THIS WEEK:** List on PigiaMe & Jiji
6. **ONGOING:** Post content regularly

**Remember:** SEO takes time, but these steps will get you indexed within days and ranking within weeks!

---

## 📍 TARGET SEARCHES TO RANK FOR

Once indexed, you'll start appearing for:

- "bookshop eastleigh"
- "hassan bookshop"
- "books eastleigh nairobi"
- "textbooks eastleigh"
- "bookstore section one eastleigh"
- "educational books nairobi"
- "where to buy books in eastleigh"

Good luck! 🎉
